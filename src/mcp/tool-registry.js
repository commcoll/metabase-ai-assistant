...
export function getToolDefinitions() {
  return enrichTools([
    ...
    },
    {
      ...
      description: 'Create a parametric question with filters, variables, and dynamic queries — supports date ranges, dropdowns, and field filters. **Two MariaDB/MySQL gotchas you may hit on ERPnext-like schemas:** (1) **Spaces in table names** — if a field-filter parameter (`field_id` set) targets a table whose name has spaces or special chars (ERPnext: `tabGL Entry`, `tabSales Invoice`), Metabase\'s SQL generator emits invalid identifiers and the dashboard query 500s. Workaround: alias the table in your SQL (`FROM \\`tabGL Entry\\` AS gl_entry`) and pass `parameters[].alias = "gl_entry.posting_date"`. The `requires_alias_for_field_filters` flag on table-discovery tools tells you when a target table needs this. (2) **Sparse JSON keys are invisible** — Metabase samples only the first 500 rows when inferring keys inside JSON columns. Rarely-set ERPnext custom fields won\'t appear in metadata and won\'t be queryable through field references. Use `JSON_EXTRACT(col, \'$.your_key\') AS your_key` directly in SQL. The `has_json_columns` flag on table-discovery tools tells you when a target table has JSON columns to be careful with.',
      inputSchema: {
        type: 'object',
        properties: {
          name: {
